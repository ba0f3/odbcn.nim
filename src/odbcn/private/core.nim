...
export
    GUID, OdbcDate, OdbcTime, OdbcTimestamp,
    # errors
    OdbcException, OdbcError, OdbcErrors,
    # dataschema
    OdbcValue, OdbcPrimType
...
proc getData[T](ds: OdbcAnyResult, colIdx: TSqlUSmallInt, ret: var T) =
    getData(SqlHandle(ds), colIdx, ret)
...
proc getDatas*[T](ds: OdbcAnyResult, ret: var T) =
    ...
    when compiles(getData(ds, 1, ret)):
        getData(ds, 1, ret)
    else:
...
template bindParam[T](x: OdbcAnyStmt, i: TSqlUSmallInt, param: T) =
    bindParam(SqlHandle(x), i, param)
...
template bindParamWrapper(stmt, idx, val) =
    {.hint[ConvFromXtoItselfNotNeeded]: off.}:
        when val is string | seq | openArray:
            bindParam(stmt, idx, val)
        elif compiles(unsafeAddr(val)):
            bindParam(stmt, idx, unsafeAddr(val))
        else:
            var param = val
...
template bindParamsOneObjSimple(stmt: OdbcAnyStmt, params) =
    for i, val in enumerate fields(params):
        let idx = TSqlUSmallInt(i + 1)
        bindParamWrapper(stmt, idx, val)

template bindParamsOneObj[T](
    stmt: OdbcAnyStmt,
    params: T,
    _: openArray[string],
) =
    bindParamsOneObjSimple(stmt, params)

macro bindParamsOneObj[T](
    stmt: OdbcAnyStmt,
    params: T,
    order: static openArray[string],
) =
...
template bindParamsOne(
    stmt: OdbcAnyStmt,
    params: typed,
    order: openArray[string],
) =
    when compiles(bindParamWrapper(stmt, TSqlUSmallInt(1), params)):
        checkNumParams(order.len, 1)
        bindParamWrapper(stmt, TSqlUSmallInt(1), params)
    else:
        bindParamsOneObj(stmt, params, order)
    ...
    for i, key, val in enumerate fieldPairs ret:
        let idx =
            when hasCustomPragma(val, odbcCol):
                ...
                TSqlUSmallInt(i + 1)
        bindCol(SqlHandle(stmt), idx, val)
